...
class ConfDB
{
    ...
    {
        $ret = [
            self::ERR_TABLENAME_MUST_STRING => '数据表名必须为字符串',
            self::ERR_TABLE_NOT_EXISTS => '数据表不存在',
            self::ERR_KEY_NOT_EXISTS => '该KEY不存在',
            self::ERR_NOT_EMPTY => '值不能为空',
            self::ERR_INFO_NO_MODIFY => '基本信息不能修改',
            self::ERR_TABLENAME_NOT_EMPTY => '表名不能为空',
            self::ERR_DB_NOT_WRITE => '数据库不可写',
            self::ERR_NO_DB => '数据库不合法',
            self::MSG_CONNECT_SUCCESS => '连接成功'
            ];
